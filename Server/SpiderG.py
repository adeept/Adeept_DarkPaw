...
def move_smooth_base(servo_name, goal_pwm, old_pwm, now_pos, total_range):
	...
	return pwm_input
def set_angle(servo_name, pwm_input):
	pwm.set_pwm(servo_name, 0, pwm_input)
...
def status_GenOut(height_input, pitch_input, roll_input):
	...
	def leg_HL_status():
		...
		goal_dict['HLM'] = ctrl_range(int(HLM_init_pwm + (height_input + HL_input)*HLM_direction), max_dict['HLM'], min_dict['HLM'])
		goal_dict['HLE'] = HLE_init_pwm

	def leg_HR_status():
		goal_dict['HRB'] = HRB_init_pwm
		goal_dict['HRM'] = ctrl_range(int(HRM_init_pwm + (height_input + HR_input)*HRM_direction), max_dict['HRM'], min_dict['HRM'])
		...
		if MPU_connection:
			try:
				...
				if abs(X_error)>mpu_tor or abs(Y_error)>mpu_tor:
					status_GenOut(0, Y_error*P, X_error*P)
					direct_M_move()
			except:
				time.sleep(0.1)
				sensor = mpu6050(0x68)
